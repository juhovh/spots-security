...
//        This worked for blue-080430
//        int trustManagerAddr = Spot.getInstance().getFlashMemoryDevice().
//                getSectorMap()[ConfigPage.TRUST_MANAGER_SECTOR];
        
        // The IFlashMemoryDevice interface changed in blue-080609
        int trustManagerAddr = Spot.getInstance().getFlashMemoryDevice().
                getSectorAddress(ConfigPage.TRUST_MANAGER_SECTOR);
        byte[] data;
        try {
            StreamConnection mem = (StreamConnection) Connector
                    .open("memory://" + trustManagerAddr);
            MemoryInputStream mis = (MemoryInputStream) mem.openInputStream();
            data = new byte[MAX_TRUSTMANAGER_SIZE];
            mis.read(data);
            return new TrustManager(data);
        } catch (IOException e) {
    ...
    public void flashTrustManager() {
        if (!Spot.getInstance().isMasterIsolate()) {
            throw new SpotFatalException("Can only flash the trust manager " +
                    "from the master isolate");
        }
        ...
        try {
           data = asByteArray();
            ...
            throw new SpotFatalException("Encountered error while flashing " +
                    "key store. " + ex.getMessage());
        }
        
        if (data == null)
            throw new SpotFatalException("Could not serialize key store.");
        
        /* NorFlashSector writes even lengths buffers */
        if ((data.length & 1) == 1) {
        ...
        if (data.length > MAX_TRUSTMANAGER_SIZE) 
            throw new SpotFatalException("TrustManager size would exceed" +
                    " allocated space. Command failed.");
        IFlashMemoryDevice flash = Spot.getInstance().getFlashMemoryDevice();
        INorFlashSector sector = new NorFlashSector(flash,
                ConfigPage.TRUST_MANAGER_SECTOR,
                INorFlashSector.SYSTEM_PURPOSED);
        sector.erase();
        sector.setBytes(0, data, 0, data.length);
    }
